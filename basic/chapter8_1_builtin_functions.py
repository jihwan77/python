...
print(all([1, 2, 0])) # False : and
print(any([1, 2, 0])) # True : or
...
print(list(filter(lambda x: abs(x) > 2, [1, -3, 2, 0, -5, 6]))) # lambda를 쓰면 함수선언 없이 사용가능

# id : 객체의 주소값(레퍼런스) 반환
print(id(int(5)))
print(id(4))

# len : 요소의 길이 반환 (__iter__)
print(len('abcdefg'))
...
print(list(zip([10, 20, 30],[40, 50, 70]))) # 원하는 데이타(list)로 형번환
print(type(list(zip([10, 20, 30],[40, 50, 70]))[0])) # <class 'tuple'>